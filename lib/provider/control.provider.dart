...
class Control with _$Control {
  factory Control({
    ...
    @Default(16)
        int frameInterval, // get image per frame
    @Default(0)
        int rotateAngle, // camera preview angle, clockwise
    @Default(false)
        bool isCameraFront, // camera position
    @Default(false)
        bool isCameraStreamStarted, // camera stream flag
    // gesture_train.page
    @Default(3)
        int makeSequenceTime, // user start make train set flag
    @Default(false)
